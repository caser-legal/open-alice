...
export class AgentCenter {
  ...
  private async *_generate(
    ...
    // 1.5. SKILL INJECTION - Auto-load relevant skills and inject into system prompt
    const skillInjection = await injectSkills(prompt, 3)
    let finalPrompt = prompt
    if (skillInjection.hasSkills && skillInjection.appendSystemPrompt) {
      console.log(`[AgentCenter] Auto-loaded ${skillInjection.loadedSkills.length} skills:`, skillInjection.loadedSkills)
      // Merge skill injection with any existing system prompt
      opts = {
        ...opts,
        appendSystemPrompt: opts?.appendSystemPrompt
          ? `${opts.appendSystemPrompt}\n\n${skillInjection.appendSystemPrompt}`
          : skillInjection.appendSystemPrompt,
      }
    }
