...
import { makeContract, resolveSymbol, mapAlpacaOrderStatus, isCryptoSymbol, normalizeSymbol } from './alpaca-contracts.js'
...
export class AlpacaAccount implements ITradingAccount {
  ...
  async searchContracts(pattern: string): Promise<ContractDescription[]> {
    ...
    // Check if it's a crypto symbol (contains / or ends with USD/USDT/USDC/BTC/ETH)
    const normalized = normalizeSymbol(upperPattern)
    if (isCryptoSymbol(normalized)) {
      // Try to validate crypto asset exists
      try {
        const assets = await this.client.getAssets({
          asset_class: 'crypto',
        }) as Array<{ symbol: string; tradable: boolean; status: string }>
        
        // Find matching crypto pair
        const match = assets.find(a => 
          a.symbol === normalized || a.symbol === upperPattern
        )
        if (match && match.tradable && match.status === 'active') {
          results.push({ contract: makeContract(normalized, this.provider) })
        }
      } catch {
        // If API fails, assume the symbol is valid (for paper trading)
        results.push({ contract: makeContract(normalized, this.provider) })
      }
    }

    // Also try as a stock symbol
    try {
      const stockAsset = await this.client.getAsset(upperPattern) as { tradable: boolean; status: string }
      if (stockAsset.tradable && stockAsset.status === 'active') {
        results.push({ contract: makeContract(upperPattern, this.provider) })
      }
    } catch {
      // Stock not found, ignore
    }

    // If nothing found, return the normalized pattern anyway (for paper trading flexibility)
    if (results.length === 0) {
      results.push({ contract: makeContract(normalized, this.provider) })
    }
  ...
  async getContractDetails(query: Partial<Contract>): Promise<ContractDetails | null> {
    ...
    return {
      ...
      validExchanges: isCrypto ? ['ALPACA'] : ['SMART', 'NYSE', 'NASDAQ', 'ARCA'],
      orderTypes: isCrypto 
        ? ['market', 'limit', 'stop_limit'] 
        : ['market', 'limit', 'stop', 'stop_limit', 'trailing_stop'],
      stockType: isCrypto ? 'CRYPTO' : 'COMMON',
    }
  }
  ...
  async getQuote(contract: Contract): Promise<Quote> {
    ...
    // Use crypto snapshot for crypto symbols
    if (isCryptoSymbol(symbol)) {
      const snapshots = await this.client.getCryptoSnapshots([symbol]) as Map<string, unknown>
      
      const snapshot = snapshots.get(symbol) as {
        LatestTrade?: { Price: number; Timestamp: string }
        LatestQuote?: { AskPrice: number; BidPrice: number; Timestamp: string }
        DailyBar?: { Volume: number }
      } | undefined
      
      if (!snapshot) throw new Error(`No crypto snapshot for ${symbol}`)
      
      return {
        contract: makeContract(symbol, this.provider),
        last: snapshot.LatestTrade?.Price ?? snapshot.LatestQuote?.AskPrice ?? 0,
