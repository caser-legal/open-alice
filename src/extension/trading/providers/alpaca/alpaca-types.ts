...
export interface AlpacaCryptoSnapshotRaw {
  LatestTrade: { p: number; t: string; s: number }
  LatestQuote: { ap: number; bp: number; t: string; as: number; bs: number }
  DailyBar: { v: number; o: number; c: number; h: number; l: number; t: string }
}
