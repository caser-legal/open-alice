<p align="center">
  <img src="docs/images/alice-full.png" alt="Open Alice" width="600">
</p>

<p align="center">
  AI trading agent for research, quant analysis, and automated trading operations.
</p>
...
### Option 2: Your own machine, your own keys

This repository does not ship exchange keys, wallet secrets, account ids, or trading history. Those files stay on your machine.
...
**Step 2: Add your own credentials locally**

Start the app, then enter keys in the Web UI (Settings / Trading). Alice writes them under `data/config/`, which is gitignored:

- `accounts.json` — your exchange or broker API key and secret
- `platforms.json` — which venues those accounts use
- `crypto.json` / `securities.json` — legacy single-account files, same rule
- `api-keys.json` — only if you switch the AI provider off the local Claude Code login

Use placeholders until you paste your own values. Do not commit `data/config/`, `data/trading/`, `data/sessions/`, `data/brain/`, heartbeat reports, or any `.env` file.

**Step 3: Start Open Alice**
```bash
pnpm dev
```
...
For detailed production deployment instructions (systemd, Docker, security hardening), see **[DEPLOY.md](DEPLOY.md)**.

## Configuration

All config lives in local `data/config/` JSON files (gitignored) with Zod validation. Missing files fall back to sensible defaults. Edit them on your machine or use the Web UI. Never commit live keys.

**AI Provider** — The default provider is Claude Code (`claude -p` subprocess). To use the Vercel AI SDK instead (Anthropic, OpenAI, Google, etc.), switch `ai-provider.json` to `vercel-ai-sdk` and add your API key to `api-keys.json`. A third option, Agent SDK (`@anthropic-ai/claude-agent-sdk`), is also available via `agent-sdk`.
